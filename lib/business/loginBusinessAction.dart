...
//  print(parsedJson["token"]);
    var tokenUser = parsedJson["tokenBusiness"];

    SharedPreferences prefs = await SharedPreferences.getInstance();

    // This saves the token into the device as token: j9384ujhsdf
    await prefs.setString("tokenBusiness", tokenUser);
    // will grab the token from the device
    String getTokenUser = (prefs.getString("tokenBusiness"));

    // show that the token is coming from device storage
    print(getTokenUser);
  } on Exception {
    var url = 'http://10.0.2.2:5000/api/login/businessLogin';
    // email has to be wrapped in " " or there will be an error
    String json = '{"email": "$email", "password": "$password"}';
    print(json);
    Map<String, String> headers = {"Content-type": "application/json"};
